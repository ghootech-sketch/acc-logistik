  ...
  return (
    <footer className="bg-brand-dark text-white">
      <div className="container-page grid gap-10 px-4 py-14 sm:px-6 md:grid-cols-3 lg:px-8">
        <div>
          <div className="text-3xl font-black text-brand-yellow">ACC</div>
          <div className="text-xs font-bold tracking-[0.35em] text-white/60">LOGISTIC</div>
          <p className="mt-4 max-w-sm text-sm text-white/70">Murah, Aman, Cepat, Tepat. Door to Door Cargo Service dari Jakarta ke Sumatera.</p>
          <div className="mt-5 flex gap-4 text-white/60">
            <a href="#" className="transition-colors hover:text-brand-yellow" aria-label="Facebook"><FacebookIcon /></a>
            <a href="#" className="transition-colors hover:text-brand-yellow" aria-label="Instagram"><InstagramIcon /></a>
            <a href="#" className="transition-colors hover:text-brand-yellow" aria-label="WhatsApp"><WhatsAppIcon /></a>
          </div>
        </div>
        <div>
          ...
          <div className="mt-4 grid gap-2 text-sm text-white/70">
            {['Beranda', 'Layanan', 'Rute', 'Ulasan', 'Kontak'].map((label, i) => (
              <Link key={label} href={['/', '/layanan', '/rute', '/ulasan', '/kontak'][i]} className="hover:text-brand-yellow transition-colors">{label}</Link>
            ))}
          </div>
        </div>
        <div>
      ...
      <div className="border-t border-brand-yellow/20 py-5 text-center text-sm text-white/50">© 2025 ACC Logistic. All rights reserved.</div>
    </footer>
  );
}
